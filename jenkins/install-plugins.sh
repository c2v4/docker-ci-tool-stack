...
doDownload() {
    ...
    retry ${CURL_RETRY:-5} curl --connect-timeout "${CURL_CONNECTION_TIMEOUT:-20}" --retry "${CURL_RETRY:-5}" --retry-delay "${CURL_RETRY_DELAY:-0}" --retry-max-time "${CURL_RETRY_MAX_TIME:-60}" -s -f -L "$url" -o "$jpi"
    return $?
}
...
retry() {
    ...
    do
        ...
        then
            ...
            echo "Attempt $attempt failed! Trying again in $attempt seconds..."
            sleep $(( attempt++ ))
        fi
    done
}
